...
var flashVer = -1; if (navigator.plugins != null && navigator.plugins.length > 0) {
    if (navigator.plugins["Shockwave Flash 2.0"] || navigator.plugins["Shockwave Flash"]) {
        ...
        if (versionRevision[0] == "d") { versionRevision = versionRevision.substring(1) } else {
...
} else { if ($.browser.msie) { var version; var axo; var e; try { axo = new ActiveXObject("ShockwaveFlash.ShockwaveFlash.7"); version = axo.GetVariable("$version") } catch (e) { } flashVer = version.replace("WIN ", "").replace(",", ".") } } flashVer = flashVer.split(".")[0]; if (jQuery) {
    (function(a) {
        a.extend(a.fn, { fileUpload: function(b) {
            if (flashVer >= 9) {
                a(this).each(function() {
                    ...
                    var d = location.pathname; d = d.split("/"); d.pop(); d = d.join("/") + "/"; var f = "&pagepath=" + d; if (settings.buttonImg) { f += "&buttonImg=" + escape(settings.buttonImg) }
                    if (settings.buttonText) { f += "&buttonText=" + escape(settings.buttonText) }
                    if (settings.rollover) { f += "&rollover=true" } f += "&script=" + settings.script; f += "&folder=" + escape(settings.folder); if (settings.scriptData) { var g = ""; for (var c in settings.scriptData) { g += "&" + c + "=" + settings.scriptData[c] } f += "&scriptData=" + escape(g) } f += "&btnWidth=" + settings.width; f += "&btnHeight=" + settings.height; f += "&wmode=" + settings.wmode; if (settings.hideButton) { f += "&hideButton=true" } if (settings.fileDesc) { f += "&fileDesc=" + settings.fileDesc + "&fileExt=" + settings.fileExt } if (settings.multi) { f += "&multi=true" }
                    ...
                    if (settings.onInit() !== false) {
                        ...
                        if (a.browser.msie) {
                            ...
                            // Esto es para que los adjuntos salgan a un lado
                            a(this).after('<div id="DivMain" style="float: left;width:' + settings.width + 'px"></div>')
                            a("#DivMain").append('<div id="' + a(this).attr("id") + 'Uploader"></div>');
                            document.getElementById(a(this).attr("id") + "Uploader").outerHTML = flashElement;

                            // Esto es para que los adjuntos salgan debajo
                        ...
                        // Esto es para que los adjuntos salgan a un lado
                        a("#DivMain").after('<div id="' + a(this).attr("id") + 'Queue" class="fileUploadQueue"></div>');
                        ...
                        if (j.data.action(j, h, i) !== false) {
                            ...
                            // Validación del tamaño permitido del archivo
                            //debugger;
                            var tamLimite = 10000 * 10000;
                            var tamArchivo = 0
                            if (tamLimite != undefined) {
                                //tamLimite = settings.sizeLimit / 1024;
                                tamLimite = (Math.round(settings.sizeLimit / 1024 * 100) * 0.01) / 1024;
                                tamArchivo = (l == "MB") ? k : k / 1024;
                                if (tamArchivo > tamLimite) {
                                    alert("El tamaño del archivo seleccionado supera los límites (" + tamLimite + " MB). Por favor seleccione un archivo mas chico.");
                                    return;
                                }
                            }
                            ...
                            if (i.name.length > 12) {
                                fileName = i.name.substr(0, 12) + "..."
                            }
                            else {
                                fileName = i.name
                            }
                            //debugger;
                            a("#" + a(this).attr("id") + "Queue").append('<div id="' + a(this).attr("id") + h + '" class="fileUploadQueueItem"><div class="cancel"><a href="javascript:$(\'#' + a(this).attr("id") + "').fileUploadCancel('" + h + '\')"><img src="' + settings.cancelImg + '" border="0" /></a></div><span FileName="' + i.name + '" class="fileName">' + fileName + " (" + k + l + ')</span><span class="percentage">&nbsp;</span><div class="fileUploadProgress" style="width: 100%;"><div id="' + a(this).attr("id") + h + 'ProgressBar" class="fileUploadProgressBar" style="width: 1px; height: 3px;"></div></div></div>')
                        }
                    ...
                    function(k, h, j, i) {
                        if (k.data.action(k, h, j, i) !== false) {
                            a("#" + a(this).attr("id") + h + " .fileName").text(i.type + " Error - " + j.name); a("#" + a(this).attr("id") + h).css({ border: "3px solid #FBCBBC", "background-color": "#FDE5DD" })
                        }
        ...
        }, getFilesNames: function(b) {
            var names = new Array();
            a("#" + a(this).attr("id") + "Queue .fileName").each(function() {
                names.push(a(this).attr("FileName"));
            })
