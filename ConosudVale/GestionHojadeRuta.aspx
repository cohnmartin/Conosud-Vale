...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function requestEnd(sender, args) {
            if (sender.__EVENTTARGET.indexOf("ExportExcel") >= 0) {

                window.open("Documentos/DDJJ-ListadoMensualLegajos.pdf", "_blank");
            }
        ...
        function AprobacionEspecial() {
            var idCabecera = "<%= IdCabecera %>";
            var idUsuario = "<%= IdUsuario %>";

            $.ajax({ type: "POST",
                data: "{ IdCabecera: '" + idCabecera + "',IdUsuario:'" + idUsuario + "'}",
                url: "GestionHojadeRuta.aspx/GrabarAprobacionEspecial",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                cache: false,
                success: function (AccionValida) { js_Aprobacion(AccionValida) },
                error: function (msg) { alert("Existe un error con la aplicación aaaa: " + msg); }
            });
        ...
        function js_AprobacionAsync() {
            ...
            if ("<%= TieneLegajos %>" == "False") {
                ...
                radconfirm('La hoja de ruta no posee legajos asociados, esta seguro de aprobar la hoja de todas formas?', confirmCallBackFn)
                return;
            }
            else {
        ...
        function js_Aprobacion(AccionValida) {
            if (AccionValida.d != undefined)
                var Resultado = AccionValida.d.split('|');
            else
                var Resultado = AccionValida.split('|');

            if (Resultado[0] == "APROBADA") {
                if (AccionValida.d != undefined) {
                    radalert("La hoja de ruta se ha aprobado correctamente.", 350, 100, "Aprobación");
                }
                else
                    radalert("La hoja de ruta se ha aprobado AUTOMATICAMENTE correctamente.", 350, 100, "Aprobación");
            }
                ...
                if (AccionValida.d != undefined)
                    radalert("La hoja de Ruta no se ha podido aprobar ya que posee items no aprobados", 350, 100, "Advertencia!");
            }
            else if (Resultado[0] == "HOJAS") {
                if (AccionValida.d != undefined)
                    radalert("La hoja de Ruta no se puede aprobar ya que las hojas anteriores no estan aprobadas.", 350, 100, "Advertencia!");
                else
                    radalert("La hoja de Ruta no se puede aprobar AUTOMATICAMENTE ya que las hojas anteriores no estan aprobadas.", 350, 100, "Advertencia!");
            }
            else if (Resultado[0] == "SUBACTUAL") {
                if (AccionValida.d != undefined)
                    radalert("La hoja de Ruta no se puede aprobar ya que las hojas del o los Sub-Contratistas no estan aprobadas, para el período actual.", 350, 100, "Advertencia!");
                else
                    radalert("La hoja de Ruta no se puede aprobar AUTOMATICAMENTE ya que las hojas del o los Sub-Contratistas no estan aprobadas, para el período actual.", 350, 100, "Advertencia!");
            }
            else {
                radalert("La hoja de Ruta no se puede aprobar ya que las hojas anteriores del contratista y sub contratista no estan aprobadas.", 350, 100, "Advertencia!");
            }
    ...
    <telerik:RadWindowManager ID="radwindowsmanager" runat="server" Skin="Sunset" Style="z-index: 99900000009">
        <Windows>
            ...
            <telerik:RadWindow ID="RadWindow2" runat="server" Behaviors="Close" Width="870" Title="Informe Histórico"
                Height="550px" Modal="true" NavigateUrl="ReporteViewerPeriodoHistorico.aspx"
    ...
    <asp:UpdatePanel ID="upEdicion" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            <cc1:ServerControlWindow ID="ServerControlWindow1" runat="server" onClientCanceled="LimpiarControles"
                ...
                <ContentControls>
                    <div id="divPrincipal" style="height: 400px">
                        <asp:HiddenField ID="hdIdHoja" runat="server" />
                        <asp:HiddenField ID="hdIdPlantilla" runat="server" />
                        <table border="0" cellpadding="0" cellspacing="0" width="750px" style="text-align: left;">
                            <tr>
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                        <tr>
                                            <td align="right" style="width: 100%">
                                                <asp:Button ID="btnAceptar" runat="server" SkinID="btnConosud" Text="Guardar Cambios" OnClientClick="return GuardarCambios(); " />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
